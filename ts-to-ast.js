...
function parseFile(filePath) {
    ...
    function serializeAST(node, indentLevel = 0) {
        ...
        (function serialize(node, indentLevel = 0) {
            appendOutput(`${' '.repeat(indentLevel * 2)}${ts.SyntaxKind[node.kind]}: ${node.getText().trim()}`);
    ...
    function processNode(node) {
        ...
        const significantNodeKinds = [ts.SyntaxKind.FunctionDeclaration, ts.SyntaxKind.ClassDeclaration, ts.SyntaxKind.InterfaceDeclaration, ts.SyntaxKind.EnumDeclaration, ts.SyntaxKind.TypeAliasDeclaration, ts.SyntaxKind.VariableStatement, ts.SyntaxKind.ModuleDeclaration, ts.SyntaxKind.ExportAssignment, ts.SyntaxKind.ExportDeclaration, ts.SyntaxKind.ImportDeclaration];
        if (significantNodeKinds.includes(node.kind)) {
            const snippet = node.getText();
            const astSegment = serializeAST(node);
            console.log(JSON.stringify({ ast: astSegment.trim(), code: snippet.trim() }));
        }
